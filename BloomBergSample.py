...
# Function to update UI elements based on new data
def update_ui():
    # Update time display
    ...
    # Update charts for AAPL in detail and charting tabs
    if dpg.does_item_exist("detail_line"):
        dpg.set_value("detail_line", historical_data["AAPL"])
    if dpg.does_item_exist("main_line"):
        dpg.set_value("main_line", historical_data["AAPL"])

# UI update loop running in a separate thread to periodically refresh display
def ui_update_loop():
    while dpg.is_dearpygui_running():
        # Use invoke to ensure the UI update is done in the main thread
        dpg.invoke(update_ui)
        time.sleep(2)
